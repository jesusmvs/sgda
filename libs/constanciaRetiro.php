...
$fecha_letras= fechaALetras($fechaActual);	
...
<html>
	...
	<body>
		...
		<div class="documento"><h2><strong>CONSTANCIA DE RETIRO</strong></h2></div></br>
		<p class="principal">
		Quien suscribe, <strong>'.$nombreDirector.' '.$apellidoDirector.'</strong>, titular de la cédula de identidad <strong>Nº '.$cedulaDirector.'</strong>, Director(a) de la '.$nombreInstitucion.', resolución de funcionamiento  código <strong>Nº '.$idInstitucion.'</strong>, por medio de la presente hace constar que ';
	if ($idSexo == 1) {
	 	$html .= 'el';
	 } else{
	 	$html .= 'la';
	...
	$html .=' <strong>N°: '.$cedulaAlumno.'</strong>, de <strong>'.$edad.'</strong> años de edad, natural  de  la  ciudad  de: <strong>'.$ciudad.'</strong>, estado: <strong>'.$estado.'</strong>; cursó el <strong>'.$descripcionGrado.'</strong> de  Educación ';
	if ($descripcionGrado == "3er Nivel") {
		$html .= 'Preescolar';
	} else {
		$html .= 'Básica';
	}
	$html .= ' en el periodo escolar <strong>'.$periodo.'</strong>, se  retira  de  esta  Institución   el  día <strong>'.$fechaActual.'</strong>, por <strong>'.$motivoRetiro.'</strong>, según lo ha manifestado su representante legal.<br><br></p>
		<p class="principal">
...
$dompdf->render();
$dompdf->stream("Constancia Retiro -".$nombreAlumno." ".$apellidoAlumno.".pdf", array("Attachment" => 0));
?> 